...
assert label_type in ['Chemical', 'Disease']
...
if label_type =='Disease':
    from pre_defined_disease_variables import *
elif label_type=='Chemical':
    from pre_defined_chemical_variables import *

seeds = get_seed_list(dataset, rule_type)

pos_cnt = Counter()
for sent in dev_data:
    pos_cnt += collect_POS(sent, label='I-'+label_type)
...
if rule_type=='SurfaceForm':
    ...
    suffix_dict = collect_suffix_candidates(train_data, [3,4,5], exception_list=exception_list)
    ...
    prefix_dict = collect_prefix_candidates(train_data, [4,5,6], exception_list=exception_list)
    ...
    prengram_dict = collect_inclusive_preNgram_candidates(surface_candidates, ngram_list=[1,2])
    ...
    prengram_dict = collect_exclusive_preNgram_candidates(train_data, ngram_list=[2,])
...
lf = None
if rule_type=='SurfaceForm':
    lf = CustomizedDictionaryMatcher("CoreDictionaryUncased",list(candidates_final),uncased=True,i_label='I-'+label_type, match_lemmas=True)
elif rule_type=='Suffix':
    lf = CustomizedCommonSuffixes(candidates_final, threshold=7, label_type='I-'+label_type)
elif rule_type=='Prefix':
    lf = CustomizedCommonPrefixes(candidates_final, threshold=5, label_type='I-'+label_type)
elif rule_type=='InclusivePostNgram':
    lf = CustomizedInclusivePostNgram(tuple(candidates_final), label_type='I-'+label_type, length_list=[1,2,3])
elif rule_type=='InclusivePreNgram':
    lf = CustomizedInclusivePreNgram(tuple(candidates_final), label_type='I-'+label_type, length_list=[1,2])
elif rule_type=='ExclusivePreNgram':
    lf = CustomizedExclusivePreNgram(tuple(candidates_final), label_type='I-'+label_type, length_list=[2,])
elif rule_type=='Dependency':
    lf = CustomizedDependency("Dependency",list(candidates_final),uncased=True,i_label='I-'+label_type, match_lemmas=True)

cand2emb = extract_candidate_embeddings(train_data, lf)

################# Write postive seeds
pos_seeds = seeds
with open('{}/{}_{}_Pos_Seeds.pk'.format(output_dir, label_type, rule_type), 'wb') as fw:
...
with open('{}/{}_{}_Neg_Seeds.pk'.format(output_dir, label_type, rule_type), 'wb') as fw:
...
with open('{}/{}_{}_node2idx.pk'.format(output_dir, label_type, rule_type), 'wb') as fw:
...
print(nodes.shape)
with open('{}/{}_{}_node_embeddings.pk'.format(output_dir, label_type, rule_type), 'wb') as fw:
...
fname = '{}/{}_{}_edges.pk'.format(output_dir, label_type, rule_type)
