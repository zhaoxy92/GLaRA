...
import string
from pre_defined_chemical_variables import get_exception_list
...
def collect_suffix_candidates(data, len_list, exception_list):
    suffix_dict = Counter() 
    for sent in data:
        for tk in sent['tokens']:
            lemma = tk.lemma_.lower()
            if len(lemma)>5 and tk.pos_=='NOUN' and \
                    not lemma in exception_list:
                for suffix_len in len_list:
                    suffix_dict[lemma[-suffix_len:]] +=1
    return suffix_dict 

def collect_prefix_candidates(data, len_list, exception_list):
    prefix_dict = Counter() 
    for sent in data:
        for tk in sent['tokens']:
            lemma = tk.lemma_
            if len(lemma)>3 and tk.pos_=='NOUN' and not lemma.lower() in exception_list:
...
def collect_inclusive_postNgram_candidates(candidate_set, ngram_list):
    ...
    for ngram in ngram_list:
        for item in candidate_set:
            if len(item.split())>ngram and \
              not item.endswith(('ray', 'rays', '- ray', '- rays', '- beta', '- alpha', 'loading', 'cell')) and\
              not item.startswith(tuple(string.punctuation)) and not item.startswith(tuple(string.digits)):
                ngram_dict[' '.join(item.split()[-ngram:])] +=1
    return ngram_dict


def collect_inclusive_preNgram_candidates(candidate_set, ngram_list):
    ngram_dict = Counter()
    for ngram in ngram_list:
        for item in candidate_set:
            if len(item.split())>ngram and not item.split()[0].lower() in ['similar', 'significant']:
                ngram_dict[' '.join(item.split()[:ngram])] +=1
    return ngram_dict



def collect_exclusive_preNgram_candidates(data, ngram_list=[2,]):
    prengram_dict = Counter()
    for sent in data:
        tokens = sent['tokens']
        i = 1
        while i<len(tokens):
            if tokens[i].pos_ in [ 'NOUN'] and tokens[i-1].pos_ not in ['NOUN']:
                right= i
                for ngram in ngram_list:
                    left = max(0, i-ngram)
