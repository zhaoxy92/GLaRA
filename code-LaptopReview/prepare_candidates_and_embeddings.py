...
if rule_type=='SurfaceForm':
    ...
    suffix_dict = collect_suffix_candidates(train_data, [3,4,5])
    ...
    prefix_dict = collect_prefix_candidates(train_data, [3,4,5])
    ...
    prengram_dict = collect_exclusive_preNgram_candidates(train_data,ngram_list=[1,2,3])
...
if rule_type=='SurfaceForm':
    ...
    lf = CustomizedCommonSuffixes(candidates_final, threshold=3)
elif rule_type=='Prefix':
    lf = CustomizedCommonPrefixes(candidates_final,  threshold=3, match_lemma=False, exceptions=set())
elif rule_type=='InclusivePostNgram':
    lf = CustomizedInclusivePostNgram(tuple(candidates_final), length_list=[1,2,3])
elif rule_type=='InclusivePreNgram':
...
if rule_type=='SurfaceForm':
    for w in cand2emb:
        if w.split()[0] in ["a", "and", "as", "be", "but", "do", "even","for", "from", "had", "has", "have", "i", "in", 
                            "is", "its", "just", "my", "no", "not", "of", "on", "or","that", "the", "these", "this",
                            "those", "to", "very","what", "which", "who", "with", "laptop", "computer", "pc"]:
            neg_seeds.add(w)
