...
def collect_suffix_candidates(data, len_list, exceptions):
    suffix_dict = Counter() 
    for sent in data:
        for tk in sent['tokens']:
            lemma = tk.lemma_
            if len(lemma)>3 and tk.pos_=='NOUN' and not lemma in exceptions and \
                    not lemma.endswith(('lly', 'ted', 'ary', 'osis', 'asis')):
                for suffix_len in len_list:
                    suffix_dict[lemma[-suffix_len:]] +=1
    return suffix_dict 

def collect_prefix_candidates(data, len_list, exceptions):
    prefix_dict = Counter() 
    for sent in data:
        for tk in sent['tokens']:
            lemma = tk.lemma_
            if len(lemma)>5 and tk.pos_=='NOUN' and not lemma.lower() in exceptions :
...
def collect_inclusive_postNgram_candidates(candidate_set, ngram_list):
    ...
    for ngram in ngram_list:
        for item in candidate_set:
            if len(item.split())>ngram and not item.startswith(('for','to','a','the','cut','quality')) and \
                 not ',' in item and not '+' in item and not '=' in item and \
            not '(' in item and not ')' in item and not '.' in item and not ';' in item and \
...
def collect_exclusive_preNgram_candidates(data, ngram_list=[1,2,3], exceptions=[]):
    ...
    for sent in data:
        ...
        while i<len(tokens):
            if tokens[i].pos_ in [ 'NOUN'] and tokens[i-1].pos_ not in ['NOUN'] and \
                    not tokens[i].lemma_ in exceptions:
                right= i
                for ngram in ngram_list:
                    left = max(0, i-ngram)
